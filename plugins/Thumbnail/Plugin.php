...
class Thumbnail_Plugin implements Typecho_Plugin_Interface
{
	...
	public static function show($obj)
	{
		...
		$default_img = $url_plugin . "default_" . mt_rand(1,5) . ".jpg";
				...
				*****/
				...
				foreach ($attach as $attach) {
					...
					if (Thumbnail_Plugin::isImage($attach_text['type'], $arr_ext)) {
						...
						$img = Typecho_Common::url($attach_text['path'], 'http://bcs.duapp.com/' . $bucket);
						break;
					}
				}
			}
			$img = '<a href="' . $obj->permalink . '" title="' . $obj->title . '"><img class="thumbnail" src="' . $img . '" width=160 height=120></a>';
		} else {
			$img = "";
		}
