...
class ArticleList implements Typecho_Plugin_Interface
{
    ...
    public static function config(Typecho_Widget_Helper_Form $form) {
        $numset = new Typecho_Widget_Helper_Form_Element_Radio('numset', array('a' => '与Blog设置中的"文章列表数目"相同', 'b' => '单独设定文章列表数目'), 'a', '文章数目选项');
        ...
        $form->addInput($rndnum->addRule('required', _t('必须填写文章列表数目'))->addRule('isInteger', _t('文章数目必须是纯数字')));
        ...
        $form->addInput($listnum->addRule('required', _t('必须填写文章列表数目'))->addRule('isInteger', _t('文章数目必须是纯数字')));
        ...
        $mode = new Typecho_Widget_Helper_Form_Element_Radio('mode', array('all' => '所有分类', 'manul' => '选择分类'), 'all', '随机日志列表');
        $form->addInput($mode);
        
        $db1 = Typecho_Db::get();
        $test = $db1->fetchAll($db1->select('table.metas.mid', 'table.metas.name')->from('table.metas')->where('table.metas.type = ?', 'category'));
        $a = array();
        foreach ($test as $item) {
            $a[$item['mid']] = ($item['name']);
        }
        
        $category = new Typecho_Widget_Helper_Form_Element_Checkbox('category', $a, array(), _t('分类显示随机日志'));
     ...
     * @param string $format
     */
    public static function hot($format = '<li><a href="{permalink}"><span style="color:#f9962b">{views}℃</span>&nbsp;{title}</a></li>') {
        ...
        $db = Typecho_Db::get();
        $rst = $db->fetchAll($db->select('cid', 'title', 'slug', 'type', 'views')->from('table.contents')->where('table.contents.status = ?', 'publish')->where('table.contents.type = ?', 'post')->order('table.contents.views', Typecho_Db::SORT_DESC)->limit($num));
        echo '<div class="hot-list">' . '<h5>热门文章</h5>' . '<ul>';
        foreach ($rst as $result) {
            ...
            echo str_replace(array('{permalink}', '{title}', '{views}'), array($value['permalink'], $title, $value['views']), $format);
        }
        echo '</ul></div>';
    }
    ...
    public static function random($format = '<li><a href="{permalink}">{title}</a></li>') {
        ...
        if ($xml1 && $option->rndtime != 0 && time() - $xml1->attributes() < $option->rndtime) {
            ...
            /**获取日志总数*/
            if ($option->mode == 'all') {
                $rs = $db->fetchRow($db->select(array('COUNT(cid)' => 'total'))->from('table.contents')->where('table.contents.status = ?', 'publish')->where('table.contents.type = ?', 'post'));
            } else {
                $category = implode(",", $option->category);
                $sql = 'table.relationships.mid in (' . $category . ')';
                $rs = $db->fetchRow($db->select(array('COUNT(table.contents.cid)' => 'total'))->from('table.contents')->join('table.relationships', 'table.relationships.cid = table.contents.cid', Typecho_Db::INNER_JOIN)->where('table.contents.status = ?', 'publish')->where($sql));
            }
            $total = $rs['total'];
            ...
            if ($option->mode == 'all') {
                foreach ($rand as $index) {
                    $result = $db->fetchRow($db->select('cid', 'title', 'slug', 'type')->from('table.contents')->where('table.contents.status = ?', 'publish')->where('table.contents.type = ?', 'post')->offset($index)->limit(1));
                ...
                foreach ($rand as $index) {
                    $result = $db->fetchRow($db->select('table.contents.cid', 'table.contents.title', 'table.contents.slug', 'table.contents.type')->from('table.contents')->join('table.relationships', 'table.relationships.cid = table.contents.cid', Typecho_Db::INNER_JOIN)->where('table.contents.status = ?', 'publish')->where($sql)->offset($index)->limit(1));
